...
IOWA.CountdownTimer.Band.prototype.getColor = function (t) {
  ...
  for (e = 0; e < this.colors.length; e++)
    if (
      ((this.colors[e].ratio = this.colors[e].size / n),
...
IOWA.CountdownTimer.Band.prototype.update = function () {
  if (this.isPlaying || this.parent.drawAll || this.needsRedraw) {
    ...
    t.clearRect(
      this.center.x - this.radius - e / 2,
      this.center.y - this.radius - i / 2,
      2 * this.radius + e,
      2 * this.radius + i
    );
    for (
      ...
      var a = o < s.length - 1 ? o + 1 : 0,
        l =
          this.radius * (r[a].x + (s[a].x - r[a].x) * this.posShift) +
          this.center.x,
        A =
          this.radius * (r[a].y + (s[a].y - r[a].y) * this.posShift) +
          this.center.y,
      ...
      if (u === n) t.lineTo(l, A);
        ...
        var c =
            this.radius * (r[o].x + (s[o].x - r[o].x) * this.posShift) +
            this.center.x,
          d =
            this.radius * (r[o].y + (s[o].y - r[o].y) * this.posShift) +
            this.center.y;
    ...
    (t.strokeStyle = n),
      ...
      this.strokeOffset > s.length
        ? (this.strokeOffset = 0)
        : this.strokeOffset < 0 && (this.strokeOffset = s.length - 1),
      t.restore(),
      (this.needsRedraw = !1);
  }
};
IOWA.CountdownTimer.Band.prototype.shudder = function (t) {
  if (!this.isPlaying && t)
    (this.isPlaying = !0), this.fade("in"), (this.isShuddering = !0);
...
IOWA.CountdownTimer.Intro.prototype.update = function () {
  ...
  if (this.count > this.radius - this.parent.strokeWeight - 0.05)
    ...
    t.save(),
      ...
      t.beginPath(),
      t.arc(
        this.circle.x + this.center.x,
        this.circle.y + this.center.y,
        this.radius,
        0,
        2 * Math.PI,
        !1
  ...
  for (var n = 0; n < this.rectangles.length; n++) {
    t.beginPath(),
      t.moveTo(
        this.rectangles[n][0].x * this.radius + this.center.x,
        this.rectangles[n][0].y * this.radius + this.center.y
      );
    for (var r = 1; r < this.rectangles[n].length; r++)
      t.lineTo(
        this.rectangles[n][r].x * this.radius + this.center.x,
        this.rectangles[n][r].y * this.radius + this.center.y
      );
    t.lineTo(
      this.rectangles[n][0].x * this.radius + this.center.x,
      this.rectangles[n][0].y * this.radius + this.center.y
...
IOWA.CountdownTimer.Core = function (t, e) {
  ...
  this.isMobile =
    this.containerDomElement.offsetWidth <=
    IOWA.CountdownTimer.MOBILE_MAX_BREAKPOINT;
  this.firstRun = !0;
  this.introRunning = !1;
...
IOWA.CountdownTimer.Core.prototype.play = function (t) {
  this.isPlaying ||
    (this.isReady || this.setUp(t), (this.isPlaying = !0), this.onFrame());
};
...
IOWA.CountdownTimer.Core.prototype.checkTime = function () {
  ...
  (this.firstRun || this.lastNumbers.days !== t.days) &&
    (this.bands[0].changeShape(Math.floor(t.days / 10)),
    this.bands[1].changeShape(t.days % 10)),
    (this.firstRun || this.lastNumbers.hours !== t.hours) &&
      (this.bands[2].changeShape(Math.floor(t.hours / 10)),
      this.bands[3].changeShape(t.hours % 10)),
    (this.firstRun || this.lastNumbers.minutes !== t.minutes) &&
      (this.bands[4].changeShape(Math.floor(t.minutes / 10)),
      this.bands[5].changeShape(t.minutes % 10)),
    (this.firstRun || this.lastNumbers.seconds !== t.seconds) &&
      (this.bands[6].changeShape(Math.floor(t.seconds / 10)),
      this.bands[7].changeShape(t.seconds % 10)),
    (this.lastNumbers = t),
    (this.firstRun = !1);
};
  ...
  if (this.isPlaying) {
    ...
    for (
      e.save(),
        ...
        e.restore(),
        (this.unitsAdded && !this.drawAll) ||
          (this.addUnits(), (this.unitsAdded = !0)),
        t = 0;
      t < this.bands.length;
      t++
    )
...
IOWA.CountdownTimer.Core.prototype.handleMouseShudder = function () {
  for (
    var t = this.mouseCoords.x, e = this.mouseCoords.y, i = 0;
    i < this.bands.length;
    i++
  )
    t > this.bands[i].center.x - this.bands[i].radius &&
    t < this.bands[i].center.x + this.bands[i].radius &&
    e > this.bands[i].center.y - this.bands[i].radius &&
    e < this.bands[i].center.y + this.bands[i].radius
      ? this.bands[i].shudder(!0)
      : this.bands[i].shudder(!1);
  this.mouseCoords = null;
};
...
IOWA.CountdownTimer.Core.prototype.getBandCenter = function (t) {
  var e,
    i,
    n,
    r = this.containerDomElement.offsetWidth,
    s =
      "horizontal" === this.format
        ? this.containerDomElement.offsetWidth / 2
        : this.containerDomElement.offsetWidth;
  return (
    "horizontal" === this.format
      ? ((n = Math.floor(t / 2)),
        (e =
          this.layout.x +
          this.layout.radius +
          2 * this.layout.radius * t +
          this.bandPadding * t +
          n * (this.bandGutter - this.bandPadding)),
        (i = this.layout.y + this.layout.radius))
      : ((n = Math.floor(t / 2)),
        (e =
          this.layout.x +
          this.layout.radius +
...
IOWA.CountdownTimer.Core.prototype.addUnits = function () {
  ...
  e.save(),
    ...
    e.fillText(
      "Days",
      this.bands[0].center.x + this.layout.radius + this.bandPadding / 2,
      this.bands[0].center.y + this.layout.radius + t
    ),
    e.fillText(
      "Hours",
      this.bands[2].center.x + this.layout.radius + this.bandPadding / 2,
      this.bands[2].center.y + this.layout.radius + t
    ),
    e.fillText(
      "Minutes",
      this.bands[4].center.x + this.layout.radius + this.bandPadding / 2,
      this.bands[4].center.y + this.layout.radius + t
    ),
    e.fillText(
      "Seconds",
      this.bands[6].center.x + this.layout.radius + this.bandPadding / 2,
      this.bands[6].center.y + this.layout.radius + t
    ),
    e.restore();
};
IOWA.CountdownTimer.Core.prototype.addSeparators = function () {
  ...
  for (var e = 0; e < this.separators.length; e++)
    t.clearRect(
      this.separators[e].x - 2,
      this.separators[e].y - 2,
      this.separators[e].w + 4,
      this.separators[e].h + 4
    ),
      t.beginPath(),
      t.moveTo(this.separators[e].x, this.separators[e].y),
      t.lineTo(
        this.separators[e].x + this.separators[e].w,
        this.separators[e].y + this.separators[e].h
      ),
...
IOWA.CountdownTimer.Core.prototype.getSeparators = function () {
  ...
  for (var t = 1; 3 >= t; t++) {
    var e =
        this.bands[2 * t].center.x -
        this.layout.radius -
        (this.bandPadding + this.bandGutter) / 2,
      i =
        this.bands[2 * t].center.y + this.layout.radius - this.bandGutter / 1.6;
...
IOWA.CountdownTimer.Core.prototype.getLayout = function () {
  ...
  t < IOWA.CountdownTimer.MOBILE_BREAKPOINT
    ? ((this.countdownMargin = 14),
      (this.bandGutter = 16),
      (this.bandPadding = 4))
    : t < IOWA.CountdownTimer.MOBILE_MAX_BREAKPOINT
    ? ((this.countdownMargin = 14),
      (this.bandGutter = 16),
      (this.bandPadding = 4))
    : t < IOWA.CountdownTimer.TABLET_BREAKPOINT
    ...
    ? ((this.countdownMargin = 4),
      ...
      (this.bandPadding = 4))
    : t > this.maxWidth &&
      ((this.countdownMargin = Math.round((t - this.maxWidth) / 2)),
      (this.bandGutter = 32),
      (this.bandPadding = 8)),
    t < IOWA.CountdownTimer.MOBILE_BREAKPOINT
      ...
      : t < IOWA.CountdownTimer.TABLET_BREAKPOINT
      ? (this.strokeWeight = 2.5)
      : t < IOWA.CountdownTimer.DESKTOP_BREAKPOINT
      ? (this.strokeWeight = 3)
      : t < IOWA.CountdownTimer.XLARGE_BREAKPOINT && (this.strokeWeight = 4);
  ...
  "horizontal" === this.format && (o -= IOWA.CountdownTimer.CENTER_OFFSET),
    "stacked" === this.format &&
      (r = (i - this.bandGutter - 2 * this.bandPadding) / 4 / 2),
    (this.layout = {
      x: s,
      y: o,
      radius: r,
...
IOWA.CountdownTimer.Core.prototype.resetCanvas = function () {
  ...
  this.intro &&
    ...
    (this.intro.center =
      "horizontal" === this.format
        ? this.getBandCenter(1)
        : this.getBandCenter(5))),
    this.getSeparators(),
